  ...
  private async startConsumer(): Promise<void> {
    ...
    connection.on('error', (error: unknown) => {
      this.logger.error(JSON.stringify(createJsonLogEntry({
        ...
        message: 'AMQP connection error for thumbnail consumer.',
    ...
    connection.on('close', () => {
      this.logger.warn(JSON.stringify(createJsonLogEntry({
        level: 'warn',
        service: 'thumbnail-service',
        message: 'AMQP connection closed for thumbnail consumer.',
        correlationId: 'system',
        queue,
      })));
    });
    channel.on('error', (error: unknown) => {
      this.logger.error(JSON.stringify(createJsonLogEntry({
        level: 'error',
        service: 'thumbnail-service',
        message: 'AMQP channel error for thumbnail consumer.',
        correlationId: 'system',
        queue,
        error,
      })));
    ...
    channel.on('close', () => {
      this.logger.warn(JSON.stringify(createJsonLogEntry({
        level: 'warn',
        service: 'thumbnail-service',
        message: 'AMQP channel closed for thumbnail consumer.',
        correlationId: 'system',
        queue,
      })));
    });
    ...
    this.logger.log(JSON.stringify(createJsonLogEntry({
      level: 'info',
      service: 'thumbnail-service',
      message: 'Thumbnail consumer started.',
      correlationId: 'system',
      queue,
      metadata: { prefetch },
    })));
