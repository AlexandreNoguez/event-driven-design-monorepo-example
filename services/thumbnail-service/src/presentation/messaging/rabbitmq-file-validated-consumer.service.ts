...
export class RabbitMqFileValidatedConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async startConsumer(): Promise<void> {
    ...
    const queue = process.env.THUMBNAIL_SERVICE_QUEUE ?? 'q.thumbnail';
    const prefetch = parsePositiveInt(process.env.THUMBNAIL_SERVICE_PREFETCH, 10);
  ...
  private async handleMessage(channel: amqp.Channel, message: amqp.ConsumeMessage): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Invalid JSON in thumbnail message. ${error instanceof Error ? error.message : String(error)}`,
      );
    ...
    if (!isFileValidatedEvent(parsed)) {
      const type = typeof (parsed as Record<string, unknown>)?.type === 'string'
        ...
        : 'unknown';
      this.logger.warn(`Unsupported event on thumbnail queue (type=${type}). Sending to retry/DLQ.`);
      channel.nack(message, false, false);
      return;
    }
