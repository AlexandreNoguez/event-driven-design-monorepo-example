...
- publishes `ProcessingCompleted.v1` through its outbox (current MVP behavior)
- runs a shadow process manager for the future Saga migration
...
The current implementation now includes a **shadow process manager** inside `projection-service`:
...
- it does **not** publish terminal events yet
- it compares the shadow result with the current `ProcessingCompleted.v1` generated by the projection flow

Shadow-mode goals:

- make state transitions explicit and queryable
- validate timeout and terminal-state rules before moving event publication away from the current MVP path
- reduce migration risk for the eventual Saga cutover

Current terminal-state ownership remains unchanged:

- `projection-service` still publishes `ProcessingCompleted.v1`
...
- implement a dedicated read/query surface for the shadow process manager if operational visibility becomes necessary
- migrate terminal event publishing from the current projection logic to the process manager after consistency validation
- add `ProcessingFailed.v1` and `ProcessingTimedOut.v1` when the Saga leaves shadow mode
